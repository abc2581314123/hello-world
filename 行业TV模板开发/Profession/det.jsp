...
<body style="width:1280px; height:720px; background: url('img/body.jpg') no-repeat">
   ...
   Authentication.CTCSetConfig("playUrl_hw",location.href);
      Authentication.CTCSetConfig("playUrl_zx",location.href);
        function play(code) {
            // 为多集单集区分  ，单集传入""
            if ("" == code){
        ...
        function keyBack(){
            var b = backListUrl.replace(/\*/g,"&")
            // if(b.lastIndexOf('&gopage=') == -1) {
            //     backListUrl = b + '&gopage='+fnGetUrlPara('gopage') + '&gonum='+fnGetUrlPara('gonum') + '&goid='+fnGetUrlPara('goid');
            // } else {
            //     backListUrl = b.substring(0,b.lastIndexOf('&gopage=')) + '&gopage='+fnGetUrlPara('gopage') + '&gonum='+fnGetUrlPara('gonum') + '&goid='+fnGetUrlPara('goid');
            // }
            
            window.location.href = backListUrl.replace(/\*/g,"&");
        }
    </script>
</body>
</html>