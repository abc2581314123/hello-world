...
<%@ page language="java" import="org.json.*" %>
    ...
    if ("".equals(providerid) || providerid == null) {
        ...
        String goidLN = "";
            if(request.getParameter("goidLN")!=null){



            goidLN = request.getParameter("goidLN");
...
<head>
    ...
    <script type="text/javascript">
        ...
        // console.log(<%=finallist.size()%>);
         ...
         if(sessionStorage.getItem("goidLN")) {
             if(sessionStorage.getItem("gopage") > 0) {
                setTimeout(function(){
                    getTotal(sessionStorage.getItem("goidLN"))
                    changeProgramForPagination(sessionStorage.getItem("goidLN") ,sessionStorage.getItem("gopage"))
                }, 300)
                
                } else {
                      changeProgram(goidLN);   
                }
            
            }else{
                if(sessionStorage.getItem("gopage") > 0) {
                setTimeout(function(){
                    getTotal(<%=finallist.get(0).get("primaryid")%>)
        ...
        function changeProgram(primaryid){
            ...
            path = "http://202.97.183.28:9090/cms/trade_programList.do?pvo.category_id="+primaryid+"&pvo.pageid=1&pvo.pagecount=10&pvo.providerid=<%=providerid%>&time=<%=time%>&riddle=<%=riddle%>";
            // console.log(path);
                ...
                if(XMLHttpRequestObject.readyState==4&&XMLHttpRequestObject.status==200){
                    ...
                    if(dataValue1 != null && dataValue1 != "") {
                        if(dataValue1!="null"){
                            ...
                            //         if(area[100].ele[j].dataset['id'] == sessionStorage.getItem("goidLN")) {
                            ...
                            currentPage = 1;
                            sessionStorage.setItem("goidLN", primaryid);
        ...
        function changeProgramForPagination(primaryid,currentPage){
            ...
            path = "http://202.97.183.28:9090/cms/trade_programList.do?pvo.category_id="+primaryid+"&pvo.pageid="+currentPage+"&pvo.pagecount=10&pvo.providerid=<%=providerid%>&time=<%=time%>&riddle=<%=riddle%>";
            // console.log(path);
            XMLHttpRequestObject.open("GET", path,true);
            XMLHttpRequestObject.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
            XMLHttpRequestObject.onreadystatechange = function() {
...
function adpLength(){
    if(adp.length <= 5) {
        document.getElementsByClassName('move_main')[0].style.width = adp.length * (adp[1].offsetWidth + 16) + 'px';
    }else {
...
<body style="width:1280px; height:720px; background: url('img/body.jpg') no-repeat">
    ...
    <div style="position: relative; margin:160px 0 0 60px">
        <!-- 看电视 -->
    ...
    <div class="move">
        ...
        <!-- 上下翻页 -->
    <div class="pageUp page">上一页</div>
    <div class="pageDown page">下一页</div>

	<script src="EPGjs/keyPress_list.js"></script>
	<script src="EPGjs/master.js"></script>
	<script src="EPGjs/list_ln.js?v=3"></script>
...
<script src="http://218.24.37.2:81/templets/epg/js/data/ad/iptv_hyls.js" charset="GB2312"></script>
	<script type="text/javascript">
   function keyBack(){
   	var localhref = location.href.replace("list","professionEPG.jsp");
window.location.href = "professionEPG.jsp";
