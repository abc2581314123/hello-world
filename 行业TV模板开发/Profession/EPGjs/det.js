...
pageControl.ok=function() {
    if (area.current == 100) {
        if (seriesflag == 1) {
            // 多集
            console.log(current.ele.dataset['code'])
            play(current.ele.dataset['code']);
        } else {
            // 单集
            play("");
        }
        ...
        if (current.index == 1) {
            ...
            var codeTemp = document.getElementsByClassName('ser')[0].getElementsByTagName('li')[0].getAttribute("data-code");
            // 播放按钮 默认第一集
