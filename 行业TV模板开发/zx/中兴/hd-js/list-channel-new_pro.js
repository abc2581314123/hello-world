...
function keyRight(){
	...
	//没有菜单
	if(menu.current==-1){
		// program.current.channelIndex===program.total-1&&program.current.subIndex===1
...
function keyUp(){
	...
	if (menu.current==-1) {
		...
		if(currentMenu.selIndex>0){
			...
			//channel总行数
			channelRow = Math.ceil(channelLen/3);
			//每次移动的距离
...
window.onload=function(){
    ...
    var zxdownindex = Util.getURLParameter("zxdownindex",vodUrl)==""?0:Util.getURLParameter("zxdownindex",vodUrl);
		...
		isAnimat = false;	
		  ...
		  var fright = totalmove%3;
		  var sdown = (totalmove-fright)/3;
...
function loadcurlinenumber(tolnum){
	var curnum = tolnum+1;
	var curnumber = Math.ceil(curnum/3);
