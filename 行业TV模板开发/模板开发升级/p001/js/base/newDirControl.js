...
* @description: 更新了 元素指定移动区域为相同区域光标不懂的情况 增加了变量isSameArea
**/	
...
Nearest.prototype.rightN = function(){
	...
	if(bytueTools.findMparaentNode(pageControls.ele).dataset['larea'] || pageControls.ele.dataset['larea']) {
		...
		if(larea != pageControls.ele.dataset['area']){
			...
			var leftArea = document.getElementById(larea);
			var leftAreaMoves = leftArea.getElementsByClassName('move');
			for(var i = 0, len = leftAreaMoves.length; i < len; i++){
				var il = downAreaMoves[i].dataset['il'];
	...
	if(minRValue == 100000 && isSameArea){
		return pageControls.index;
	}
...
Nearest.prototype.leftN = function(){
	...
	if(bytueTools.findMparaentNode(pageControls.ele).dataset['rarea'] || pageControls.ele.dataset['rarea']) {
		...
		if(rarea != pageControls.ele.dataset['area']){
			...
			var rightArea = document.getElementById(rarea);
			var rightAreaMoves = rightArea.getElementsByClassName('move');
			for(var i = 0, len = rightAreaMoves.length; i < len; i++){
				var ir = downAreaMoves[i].dataset['ir'];
				if((ir * 1) <= (domL * 1) && rightAreaMoves[i].dataset['area'] == pageControls.ele.dataset['area']){
					if(ir * 1 >= maxLValue){
						maxLValue = rightAreaMoves[i].dataset['ir'] * 1;
					}
				}
			}
			isSameArea = true;
...
Nearest.prototype.downN = function(){
	...
	if(bytueTools.findMparaentNode(pageControls.ele).dataset['darea'] || pageControls.ele.dataset['darea']) {
		...
		if(darea != pageControls.ele.dataset['area']){
			...
			var downArea = document.getElementById(darea);
			var downAreaMoves = downArea.getElementsByClassName('move');
			for(var i = 0, len = downAreaMoves.length; i < len; i++){
				var it = downAreaMoves[i].dataset['it'];
				if((it * 1) >= (domD * 1) && downAreaMoves[i].dataset['area'] == pageControls.ele.dataset['area']){
					if(it * 1 < minDValue){
						minDValue = downAreaMoves[i].dataset['it'] * 1;
					}
				}
			}
			isSameArea = true;
	...
	if(minDValue == 100000 && isSameArea){
		return pageControls.index;
	}
	// 判断边界值
	if(minDValue == 100000 && !isSameArea){
...
Nearest.prototype.upN = function(){
	...
	if(bytueTools.findMparaentNode(pageControls.ele).dataset['uarea'] || pageControls.ele.dataset['uarea']) {
		...
		if(uarea != pageControls.ele.dataset['area']){
			...
			var upArea = document.getElementById(uarea);
			var upAreaMoves = upArea.getElementsByClassName('move');
			for(var i = 0, len = upAreaMoves.length; i < len; i++){
				var idd = upAreaMoves[i].dataset['idd'];
				if((idd * 1) <= (domT * 1) && upAreaMoves[i].dataset['area'] == pageControls.ele.dataset['area']){
					if(idd * 1 >= maxDValue){
						maxDValue = upAreaMoves[i].dataset['idd'] * 1;
					}
				}
			}
			isSameArea = true;
