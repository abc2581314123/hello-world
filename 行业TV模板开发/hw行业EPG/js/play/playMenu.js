	...
	if(key=='right'){
		...
		if(menu.current==1&&menu.currentInScreen==1){
			...
			fnElesAnimat(menu.container,'left',0,-105,-animatParas.moveUnit.short,menu.menus[2].container,'width',0,300,animatParas.moveUnit.middle,function(){
                ...
                //日期 当天
            	//var datas=year+""+month+""+day;
                //getProgramInfo(0,chanids,0,datas);
		...
		if(currentMenuListEleIndex>0){
			...
			if(menu.current==0||menu.current==3){
                ...
                if(menu.current==3)	
            	{
                	...
                	showJz();
                    getProgramInfo(currentMenuListEleIndex,chanids,0,datas);  
			...
			if(menu.current==1)	
        	{
                    var listindexs=currentMenuListEleIndex
					//if(currentMenuListEleIndex>0)
					//{
					//listindexs=currentMenuListEleIndex-1
					//}
	                var chanids=currentMenuListEles[listindexs].getElementsByClassName('menu-list-channel-type')[0].innerHTML;
	                //日期 当天1
	            	var datas=year+""+month+""+day;
					showJz();
	                getProgramInfo(0,chanids,0,datas);  
	...
	if(key=='down'){
		...
		if(selMenuListEleIndex<selMenuListEles.length-1){//不是列表最后一个元素
			...
			if(menu.current==0||menu.current==3){
                ...
                showJz();
				getProgramInfo(selMenuListEleIndex,chanids,0,datas);
                timerLoadData5 = setTimeout(function() {
	            //铺数据
				if(results=="1"){
...
//				fnLayoutMenu(menu.menus[2],dataProgram.currentIndex,RenderMenuList(dataProgram,htmlTemplates[2]));
	...
	var GOTOType =Authentication.CTCGetConfig("STBType");
   		//如果是烽火盒子  页面重新加载
   		//if(GOTOType.indexOf("HG680-RLK9H-12")>=0)
   		//	{
	...
	if(key=='ok'){
		...
		top.location.href="ChanDirectAction.jsp?chanNum="+chanid;
		}
		if(menu.current==2){
...
function getChannelInfo(typeid){
    ...
    var programname=data[i].progName;
		...
		if(data[i].channelId==chanId)
			{
			xz=i;
			}
	     var temp = {name:data[i].channelName,program:programname,cid:data[i].channelIndex,t:data[i].channelId};
