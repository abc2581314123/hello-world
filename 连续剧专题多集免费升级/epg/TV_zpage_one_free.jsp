 ...
 String codeList="";//猜你喜欢code集合
	...
	try {
	   ...
	   logger.info("tv_zpage_one_dj.jsp finalres================="+finalres);
		...
		logger.error("TV_zpage_one_free.jsp页面：异常返回信息=||"+e.getMessage()+"||");
...
logger.info("TV_zpage_one_free.jsp finallist===================="+finallist);
	...
	logger.info("TV_zpage_one_free.jsp 猜你喜欢  url:"+url); 
	...
	logger.info("TV_zpage_one_free.jsp:解析接口返回JSON数据"+res);
	...
	logger.info("TV_zpage_one_free.jsp中的"+"ipaddress="+ipaddress+"usermac="+usermac+"userip="+userip+"useradsl="+useradsl);
	...
	if("".equals(temptoken)){
		...
		if(temptoken != null && !"".equals(temptoken)){
			...
			logger.info("TV_zpage_one_free.jsp session中的temptoken======================================"+session.getAttribute("buytemptoken"));
	...
	if (!"".equals(jqres2)) {
		...
		try {
			...
			logger.info("TV_zpage_one_free.jsp 获取产品列表成功，解析返回值"+resJson2);
		} catch (JSONException e) {
			logger.error("TV_zpage_one_free.jsp页面：异常返回信息=||"+ e.getMessage() + "||");
			e.printStackTrace();
		}
	} else {
		logger.info("TV_zpage_one_free.jsp 获取产品列表失败");
	...
	logger.info("TV_zpage_one_free.jsp 播放鉴权开始");
	...
	if("2".equals(jqres)){
		...
		logger.info("TV_zpage_one_free.jsp 播放鉴权成功，解析返回值========="+jqres);
		...
		logger.info("TV_zpage_one_free.jsp 播放鉴权失败================================"+jqres);
   ...
   logger.info("TV_zpage_one_free.jsp payflagpayflag================================"+payflag);


   logger.info("TV_zpage_one_free.jsp jqresjqresjqres================================"+jqres);
	...
	if("on".equals(payflag) && !"2".equals(jqres)){
		...
		if(!"".equals(userip) && !"".equals(ipaddress)){
			...
			logger.info("TV_zpage_one_free.jsp 认证结果最终========================"+rzlastres+"============================"+rzlastres);
...
/*   				//为推送信息制作返回地址
	...
	+"$backflag=0$foucsid=dy_page_right0"+"$backurlnosession="+backurlnosession.replace("&","$");
   	...
   	logger.info("TV_zpage_one_free.jsp 详情页最后的providerid====================="+providerid);

	logger.info("TV_zpage_one_free.jsp zxplayurl====================="+zxplayurl);
	logger.info("TV_zpage_one_free.jsp sosoflag:"+sosoflag); 

    logger.info("TV_zpage_one_free.jsp categoryID====================="+category_id_get);  
...
String SPRequest = "";
    ...
    logger.info("TV_zpage_one_free.jsp SPRequest================"+SPRequest);
    //测试写成1
  	jqresult = "1";
			...
			<div class="program-container">
                ...
                <%}%>
                       <%}if ("1".equals(jqresult)){%>
                       		<!-- 如果存在多级免费集数不显示试看 -->
                       		<%if(finala.get("freecount")==null || "0".equals(finala.get("freecount"))||
                       		finala.get("freecount")=="null"||finala.get("freecount")==""){
                       		%>
